...
} from "@/lib/calendar-reminder-time";
...
export async function POST(req: Request) {
  try {
    ...
    const { data: calendarItems, error: calendarError } = await supabaseServer
    ...
    const { data: existingReminders, error: existingError } = await supabaseServer
      .from("reminders")
