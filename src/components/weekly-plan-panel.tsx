...
export function WeeklyPlanPanel() {
  ...
  const loadApprovedIdeas = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/ideas/list?userId=${encodeURIComponent(userId)}&profileId=${encodeURIComponent(activeProfileId)}`
      );
  ...
  const loadCurrentSelection = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/weekly-plan/current?userId=${encodeURIComponent(userId)}&profileId=${encodeURIComponent(activeProfileId)}`
      );
      const data = await response.json();
  ...
  }, [activeProfileId, loadApprovedIdeas, loadCurrentSelection, userId]);
